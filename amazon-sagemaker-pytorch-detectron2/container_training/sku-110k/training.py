...
def update_report_file(
    ...
    try:
        ...
        if e.response["Error"]["Code"] == "404":
            columns = ["date_time", "training_job_name"]
    ...
    # Add new row
    new_row = dict({"date_time": date_time, "training_job_name": training_job_name})
    ...
    print(f"Reading metrics df from -- {os.getcwd()}")
    s3.Bucket(bucket_name).upload_file("./reports.csv", object_key)

    metrics_object_key = f"{prefix}/metrics.csv"
    s3.Bucket(bucket_name).upload_file("./metrics.csv", metrics_object_key)
...
def _train_impl(args) -> None:
    ...
    * Serialize the training configuration to a JSON file as it is required for prediction
    ...
    if args.evaluation_type:
        channel_to_ds["test"] = (
            args.test_channel,
            f"{args.annotation_channel}/testing.manifest",
        )

    register_dataset(
        metadata=dataset,
        label_name=args.label_name,
        channel_to_dataset=channel_to_ds,
    )
...
def train(args: argparse.Namespace) -> None:
    ...
    # Update the Report and Metrics File
    print(f"All the environment variables - {os.environ}")
    REGION = os.environ["REGION"]
    PREFIX = os.environ["PREFIX"]
    BUCKET_NAME = os.environ["BUCKET_NAME"]
    TRAINING_JOB_NAME = os.environ["TRAINING_JOB_NAME"]
    LOGGER.info("Updating the report and metrics file")
    update_report_file(
        training_job_name=TRAINING_JOB_NAME, prefix=PREFIX, bucket_name=BUCKET_NAME
    )
