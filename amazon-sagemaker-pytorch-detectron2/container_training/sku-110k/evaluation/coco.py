...
class EvaluateObjectDetection(COCOeval):
    ...
    def summarize(self):
        ...
        def _summarize(use_ap: bool, iou_thr=None, area_rng="all", max_dets=100):
            ...
            metric_list = [title_str, iou_str, area_rng, max_dets, mean_s]

            return mean_s, metric_list

        def _summarize_detections():
            stats = np.zeros((12,))
            metric_list = []
            stats[0], lst = _summarize(True, max_dets=self.params.maxDets[0])
            metric_list.append(lst)
            stats[1], lst = _summarize(
                True, iou_thr=0.5, max_dets=self.params.maxDets[0]
            )
            metric_list.append(lst)
            stats[2], lst = _summarize(
                True, iou_thr=0.75, max_dets=self.params.maxDets[0]
            )
            metric_list.append(lst)
            stats[3], lst = _summarize(
                True, area_rng="small", max_dets=self.params.maxDets[0]
            )
            metric_list.append(lst)
            stats[4], lst = _summarize(
                True, area_rng="medium", max_dets=self.params.maxDets[0]
            )
            metric_list.append(lst)
            stats[5], lst = _summarize(
                True, area_rng="large", max_dets=self.params.maxDets[0]
            )
            metric_list.append(lst)
            stats[6], lst = _summarize(False, max_dets=self.params.maxDets[0])
            metric_list.append(lst)
            stats[9], lst = _summarize(
                False, area_rng="small", max_dets=self.params.maxDets[0]
            )
            metric_list.append(lst)
            stats[10], lst = _summarize(
                False, area_rng="medium", max_dets=self.params.maxDets[0]
            )
            metric_list.append(lst)
            stats[11], lst = _summarize(
            ...
            print(f"Saving metric df to -- /opt/ml/code ")
            df.to_csv("/opt/ml/code/metrics.csv", index=False)
            return stats
