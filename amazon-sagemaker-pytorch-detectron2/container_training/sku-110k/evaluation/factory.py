...
def generate_predictions(
    s3_pred_objects: Sequence, image_ids: Sequence[int], coco_gt, fname: str
):

    assert len(s3_pred_objects) == len(
        image_ids
    ), f"Mismatch nb of objects ({len(s3_pred_objects)}) vs nb of identifiers ({len(image_ids)})"
